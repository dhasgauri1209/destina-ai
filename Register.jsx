...
function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const registerPayload = { ...form, email: form.email.trim() };
      ...
      const userName = loginResponse.data?.data?.user_name;
      if (!token) throw new Error("Access token missing after registration.");

      localStorage.setItem("destina_token", token);
      localStorage.setItem("destina_user_name", userName || registerPayload.full_name);
      navigate("/dashboard");
    } catch (err) {
  ...
  return (
    <main className="app-container flex min-h-[82vh] items-center justify-center py-12">
      ...
      >
        <h1 className="font-heading text-3xl font-bold text-white light:text-slate-900">Create Account</h1>
        <p className="mt-2 text-sm text-slate-300 light:text-slate-600">Begin your luxury itinerary planning.</p>

        <div className="mt-6 space-y-4">
          <div>
            <label className="text-sm text-slate-200 light:text-slate-700">Full Name</label>
            <input
              type="text"
              required
              minLength={2}
